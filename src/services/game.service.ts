    ...
    async updateStateSeed(gameId: string, answers: number[]): Promise<void> {
        return (
            this.gameRepository
                ...
                .then(game => this.getCurrentStateByGame(game).then(state => this.checkStateSeed(state, game)))
                .then(game =>
                    this.gameRepository.addStateSeed(game.user_id, gameId, answers).then(state => ({ state, game }))
                )
                // .then(({ state, game }) =>
                //     state.applied >= game.nb_players ? this.gameRepository.completeState(state) : undefined
                // )
                .then(() => undefined)
        );
    }
